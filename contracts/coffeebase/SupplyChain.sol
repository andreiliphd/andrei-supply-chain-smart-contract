...
contract SupplyChain is FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  function harvestItem(uint _upc, address _originFarmerID, string memory _originFarmName, string memory _originFarmInformation, string memory  _originFarmLatitude, string memory  _originFarmLongitude, string memory _productNotes) public onlyFarmer
  {
    ...
    Item memory item =  Item(sku, _upc, msg.sender, _originFarmerID, _originFarmName, _originFarmInformation, _originFarmLatitude, _originFarmLongitude, _upc+sku, _productNotes, 0, State.Harvested, address(0), address(0), address(0));
    items[_upc] = item;
    // Increment sku
    sku = sku + 1;
  ...
  function buyItem(uint _upc) public payable onlyDistributor
    ...
    // Transfer money to farmer
        address(uint160(items[_upc].distributorID)).transfer(items[_upc].productPrice);
    // emit the appropriate event
       emit Sold(_upc);
